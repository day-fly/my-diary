...
- 모바일 대응 레이아웃
- `/write` 웹 작성 UI (카테고리: 시/에세이/일기 + 글 작성/수정/삭제 + 사진 업로드)
- 본문 입력창에서 `Ctrl/Cmd + V` 이미지 붙여넣기 업로드
- 마크다운 포스트 작성
...
- 자동으로 `assets/photos/` 경로에 업로드
- 업로드된 이미지 Markdown이 커서 위치에 자동 삽입
...
```bash
./scripts/new-post.sh "일요일의 기록" "/절대경로/사진.jpg"
```
